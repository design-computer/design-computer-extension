...
// DES-14: MutationObserver detects code blocks in ChatGPT
test.describe('code block detection on ChatGPT', () => {
  test('detects a code block already in the DOM at load time', async ({ context }) => {
    const page = await context.newPage()

    // Intercept navigation so we get a blank page under chatgpt.com/c/* origin
  ...
  test('detects a code block injected dynamically via MutationObserver', async ({ context }) => {
    const page = await context.newPage()
    ...
    // Give content script time to set up the MutationObserver
    await page.waitForTimeout(300)

    // Inject a ChatGPT-style code block — observer should pick it up
    await page.evaluate(() => {
  ...
  test('does not log the same code block twice', async ({ context }) => {
    ...
    // Wait long enough for the observer to fire multiple times if broken
    await page.waitForTimeout(1000)

    // Trigger a DOM mutation unrelated to code blocks
    await page.evaluate(() => {
