...
function AssetTile({ asset }: { asset: AssetItem }) {
  ...
  return (
    <div
      className="aspect-square flex-[1_0_0] min-w-0 relative rounded-[12px] overflow-hidden cursor-pointer"
    ...
    >
      ...
      />
      {hovered && <div className="absolute inset-0 bg-black/20 rounded-[12px]" />}
      {hovered && (
        <button
          onClick={handleCopy}
          className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-black p-[8px] rounded-full flex items-center justify-center cursor-pointer border-none"
        >
          {copied ? (
            <Check
              width={18}
...
function PickedRow({
  ...
  return (
    <div className="flex items-center gap-0.5 rounded-[14px] bg-[#F6F6F6] border-[1.5px] border-[#CCCCCC] px-3 py-2   ">
      <img
        src={pick.previewUrl}
        alt={pick.file.name}
      ...
      {pick.status === 'done' ? (
        ...
        <Loader2 width={20} height={20} strokeWidth={2} color="#999" className="animate-spin" />
      ) : pick.status === 'error' ? (
    ...
    try {
      await Promise.all(
        picks.map(async (pick) => {
          setPickStatus(pick.id, 'uploading')
          try {
